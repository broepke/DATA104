...
### LDA
Latent Dirichlet Allocation (LDA), is a generative probabilistic model for collections of text that allows sets of observations to be explained by unobserved groups[^LDA].

### Determining the Number of Topics

Perplexity is a measurement of how well a probability distribution or probability model predicts a sample.  By plotting Perplexity, we can visualize an approximate number of topics where the model stops improving[^PLEX].  
...
Upon plotting, we can see the number of topics stops improving at **10** using the VEM algorithm.

### Top-Level Topics

Next we can create a visuaization of the words the belong to these 10 LDA topics, and can inspect them to see if they make sense.

```{r}
lda <- LDA(dtm, k = 10, method="VEM", control = list(seed = 1234))
...
Based on the output, here are some of the words that stand out, potentially forming topics.
 ...
 1. Various issues regarding a software update.
 2. Battery, Charging, and Range related issues.
 3. Sentry Mode
 4. Wheel, Bumper, Drive, Sale.  Could be related to exterior issues.
 5. Full self driving.
 6. Charging, Supercharging.
 7. Battery Degradation
 8. Phone, Insurance, Full Self Driving (Upgrading to Full Self Driving potentially, which can be purchased via the mobile application.)
 9. Autopilot upgrade, warnings related to Autopilot.
 10. Autopilot upgrade, app.

With the LDA version versus the bigram modeling, it's probably a little more precise when using the bigram approach, but we do see some other topics show up like Full Self Driving, Supercharging, and Upgrades (meaning over the air software updates).  These both can be taken into account when examining product feature related issues.

# Product Feature Analysis

After analyzing the most common customer topics, we're going to look at sentiment around each of them.  Out of the originally identified topics.  We will use a specific method here referred to as a targeted dictionary analysis[^TDA], and we will look at the following:

 * **Full Self Driving**: Also known as FSD.  A much-anticipated feature that were rolled out in private beta in 2020.  There has been quite a bit of coverage of this from early adopters on YouTube.  
 * **Software - Update**: Tesla's go through regular software updates every 1-2 weeks.
 * **Mile - Range** & **Battery - Degradation**: Being a battery powered car, the range is highly discussed.
 * **Voice - Command**: Related to the feature to give the car voice commands.
...
matplot(dfmat_voice_lsd, type = "l", xaxt = "n", lty = 1, ylab = "Frequency",
