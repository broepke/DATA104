...
df_select$Discussion = removeNumbers(df_select$Discussion)
...
```{r}
tail(df_select$Discussion, n=10)
```
...
```{r}
custom_stop_words <- bind_rows(tibble(word = c("fuck", "shit", "i"), lexicon = c("custom")), stop_words)
...
```{r}
df %>%
  filter(doc_id == "doc30686" | doc_id == "doc26462" | doc_id == "doc9918") %>%
  select(Discussion)
```
...
```{r}
df %>%
  filter(doc_id == "doc11454" | doc_id == "doc18909" | doc_id == "doc19248") %>%
  select(Discussion)
```
...
bing_df %>%
  ...
  comparison.cloud(colors = c("#1F77b4", "#FF7F0E"), max.words = 150)
```


# Checking for Top-Negation Word Pairs
The method used here doesn't take into account negation of words like no, and never preceding words.  We can quickly check the top word pairs to see how they occur in our text.


```{r}
df_select$Discussion <- removeWords(df_select$Discussion, c(stopwords('english'), "i"))
```
...
ggraph(bigram_graph, layout = "fr") +
  geom_edge_link() +
  geom_node_point() +
  geom_node_text(aes(label = name), vjust = -.9, hjust = -.1) +
  theme_minimal()
```
...
```{r}
corpus <- Corpus(VectorSource(df$Discussion))
```
...
### Odd Characters
There are some odd puncutation marks that aren't being removed.  Just strip all non-ascii
