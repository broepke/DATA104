...
**Median number of Posts shown with black line**

Here we see the distribution for the number of posts that people made.  The a mean number of posts is shown in the black line of `38`.  With posts we see an incredible long and flat upper tail showing us a small number of people are posting more than 100 times for this dataset.
...
For the distribution of people in this dataset according to age, we can see that most people are in their twenty's with the next most common in their 30's.  
...
Regarding Age Groups and the number of each gender in these groups, we can see that for the most part, each age group is balanced with regards to the number of peole in each Gender.  The exception is "Other", where they appear to be in the 18-24 group, but again.  There is not enough observations in the "Other" gender. 


```{r}
# Tried this out, but not familiar with this method yet
# my_anova <- aov(data = fb, Friends ~ .)
# etaSquared(my_anova)
```
...
As a baseline, a model showing a simple linear regression between `Age` and `Friends` to use as a baseline for our multiple regression
