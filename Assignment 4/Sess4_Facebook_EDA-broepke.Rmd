...
Here we see that the distribution of number of friends is Right Skewed with a mean (solid black line) of `696`.  We can see around the upper tail of the distribution that a few people have multiple-thousands of friends, but this is not typical. `67%` of the people fall within the first and second dashed lines (`+/- 1 Standard Deviation`). `+2` and `+3` Standard Deviations also shown.  the bars to the right of the last dash line are only `0.15%` of all people in the dataset. 
...
fb %>%
    ggplot(aes(Age)) +
    geom_histogram(binwidth = 10, color = "black",fill = "gray", alpha=0.8) +
...
We're going to create a simple linear regression between `Age` and `Friends` to get a baseline R-squared value for our dataset. Based on the EDA above, there seems to be the strongest relationship between the age of a person and the number of friends they have.
