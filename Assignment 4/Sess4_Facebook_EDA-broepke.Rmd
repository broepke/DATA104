...
fb %>%
    ggplot(aes(Friends)) +
    geom_histogram(binwidth = 100, color = "blue",fill = "lightblue") +
    geom_vline(xintercept = mean(fb$Friends), lwd = 1) +
...
```
**Mean number of Friends shown with black line**

```{r}
fb %>%
    ggplot(aes(Posts)) +
    geom_density(color = "red",fill = "salmon") +
    geom_vline(xintercept = mean(fb$Posts), lwd = 1) +
...
```
**Mean number of Posts shown with black line**

```{r}
fb %>%
    ggplot(aes(Age)) +
    geom_histogram(binwidth = 10, color = "black",fill = "gray") +
    geom_vline(xintercept = mean(fb$Age), lwd = 1) +
...
Given the VIF for `Posts`, and `Replies` is greater than `5`, we should eliminated them from the model. Next, after eliminating them, we can see which variables have a *significant* effect on the model. 
...
### Summary
After running the final linear model, we will use a combination of `Age`, `Photots`, `Tags`, and `Likes` to build a predictor for `Friends`.  The R-squared value is 0.136 telling us that 13.6% of the variance in the predicted number of Friends is explained by number number `Photots`, `Tags`, and `Likes` and the person's `Age`.  
