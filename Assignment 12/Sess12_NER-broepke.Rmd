...
Perplexity is one and is used to measure how well a probability model predicts a sample. As applied to LDA, for a given value of K, one can estimate the LDA model by comparing perplexity across different models with varying k’s. The model with the lowest perplexity is generally considered the best.
...
After calculating Perplexity for the range of topics from 2-45, the topic value of `43` returned the lowest numbers.  However, this would most likely be at a level of granularity greater than we need. 
