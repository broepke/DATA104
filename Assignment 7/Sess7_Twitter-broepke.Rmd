...
```
Trump's followers are `87 million` vs. Biden's `11 million`. Followers in social media are a sign of how much reach someone has, however Twitter is an open platform so reach / read doesn't equate to how many people are actually exposed to the content.
...
This time series is plotting `5,000` tweets on an hourly basis.  The scale on the X-axis shows the daily spacing and the peaks and valleys of the data shows a frequency of activity ramping up around 8:00 am Eastern Time and then maintaining a push through 8:00 pm eastern.  
...
As suspected, The top 10 tweets show US based locations. Position `15` shows `Melbourne` as the location.  Which could be either Australia or Florida, which has the same spelling as the Australian city.  Of course, this is to be expected because it's a US election, however, the US does effect the global economy so it tends to be watched by many people around the world.
...
What we can see from the list is that two of this user's recent tweets went "viral,: one generating `~45k` retweets and another `~14.5k` retweets.  The rest are fairly benign.  The text for the two most retweeted tweets are:
...
**Note 2**: This users profile which made it to the top of this hashtag list, was created on October 16th, 2008.  I suspected that this might be a bot account, but judging by the older account it's probably a human. While this profile only has 129 followers, we can see here, just by picking up the right hashtag, a user can get themselves into the conversation.
