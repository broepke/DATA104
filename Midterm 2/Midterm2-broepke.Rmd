...
Other than the various difference in quantities of the three, they tend to remain fairly consistent over time when transformed on a logarithmic scale. With dislikes, we see a large order of magnitude shift in the early part of December, which is noticeable.
...
```
When breaking out each category, we can see that generally, the distributions have the same shape. However, we can see the relative concentration of each category. Music is the standout leader in view volume, `comedy`, `shows`, `travel & events`, and `science & technology` also have significant numbers. 
...
Here we see how mean views trend over time, based on their **trending date**, not original publish date. We can see a slight directional increase in views in later dates.  For the most part, categories appear to remain fairly consistent over time relative to each other; we can see a stand-out in January where the `People & Blogs` category shows a one-month increase.
...
The *Entertainment* category contains the largest population of likes, with *Comedy*, *Travel & Events*, *Science & Technology*, and *Shows* showing the next most. Most of these also appear to follow the same shape with slight right-skewness generally.
...
Similar to the results of the overall Views for the categories, different categories have a greater mean number of likes.  Music is the first, and *Nonprofits & Activism* a close second. This suggests that while the number of videos in these categories is smaller, the "like" activity is greater per video.
...
Like Views, many categories are consistent with a massive spike in January 2018 in the People & Blogs category. 
...
The *Entertainment* category contains the largest population of dislikes, with *Comedy*, *Travel & Events*, *Science & Technology*, and *Shows* next most. This is a similar pattern between all three categories of Views, Likes, and Dislikes. 
...
With the view on Mean Dislikes, we can see that in this case, *Nonprofits & Activism* is the far and away leader.
