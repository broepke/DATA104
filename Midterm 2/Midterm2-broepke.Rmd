 ...
 * **NOTE**: Video_ID `JkGQog0zQKn` shows the same count of `525` as the prior "$NAME?" value.
...
df %>%
  ggplot(aes(x = likes)) +
  geom_histogram(color = "plum", fill = "plum2", bins = 30) + 
  scale_x_continuous(trans='log10') +
  theme_minimal() +
  theme(
  plot.title = element_text(face = "bold")) +
...
```
 ...
 %>%
  ggplot(aes(x = trending_date, y = likes)) +
  geom_point() + 
  theme_minimal()

```{r}
df %>%
  group_by(year(trending_date), month(trending_date)) %>%
